...
def xyz_read_by_line_chunk(target, input_file='./main.qc.xyz', line_chunk_size=503):
    """@todo: Docstring for read_in_by_line_chunk.

    :input_file: @todo
    :line_chunk_size: @todo
    :returns: @todo

    """
    with open(input_file) as f:
	    for b1 in read_in_chunk(f,503):
		    target.send(b1)
...
@coroutine
def xyz_process(sink):
    """@todo: Docstring for xyz_process.
    ...
    while True:
        ...
        b1.pop(0)
        b1.pop(0)
        xyz_list = [x.split() for x in b1]
        map(methodcaller("pop", 0), xyz_list)
        xyz_list = [map(float, x) for x in xyz_list]
        point1 = xyz_list[0]
        point2 = xyz_list[1]
	(point1,point2) = minimum_image(point1,point2)
        zero_point = [(x+y)/2 for (x, y) in zip(point1, point2)]
	for index in range(len(xyz_list)):
		(point1,xyz_list[index]) = minimum_image(point1,xyz_list[index])
# relative xy positions of Ar atom to diatoms in the same plane
        xy_relative = [relative_position_within_three_point(
            x, point1, point2) for x in xyz_list]
	xy_relative_dist = [ math.sqrt(x**2+y**2) for (x,y) in xy_relative]
        xy_relative_dist_order = argsort_list(xy_relative_dist)
        sink.send([xy_relative,xy_relative_dist,xy_relative_dist_order])
...
@coroutine
def xyz_write():
    """@todo: Docstring for xyz_write.
    :returns: @todo

    """
    xy_relative_file = open('./xy_relative.out', 'w')
    while True:
        [xy_relative,xy_relative_dist,xy_relative_dist_order] = (yield)
        for order in xy_relative_dist_order:
            xy_relative_file.write(' '.join(str(x) for x in xy_relative[order])+' '.join([' ',str(xy_relative_dist[order]),str(order)]) +'\n')
...
# relative xy positions of Ar atom to diatoms in the same plane
	for index in range(len(xyz_list_gap)):
		(point1,xyz_list_gap[index]) = minimum_image(point1,xyz_list_gap[index])
        xy_relative_gap = [relative_position_within_three_point(
            x, point1, point2) for x in xyz_list_gap]
        xy_relative_gap_with_value = merge_two_tuple(
            zip(xy_relative_gap, gap_value))
	for index in range(len(xyz_list_coup)):
		(point1,xyz_list_coup[index]) = minimum_image(point1,xyz_list_coup[index])
        xy_relative_coup = [relative_position_within_three_point(
            x, point1, point2) for x in xyz_list_coup]
        xy_relative_coup_with_value = merge_two_tuple(
            zip(xy_relative_coup, coup_value))
        sink.send([xy_relative_coup_with_value, xy_relative_gap_with_value])


@coroutine
def sink():
    coup_xy_relative_with_value_file = open(
        './coup_xy_relative_with_value.out', 'w')
    ...
    while True:
        ...
        for t in coup_xy_relative_with_value:
            coup_xy_relative_with_value_file.write(
                ' '.join(str(x) for x in t)+'\n')
        for t in gap_xy_relative_with_value:
            gap_xy_relative_with_value_file.write(
                ' '.join(str(x) for x in t)+'\n')
...
if __name__ == '__main__':
    ...
    # fig.show()
	xyz_read_by_line_chunk(xyz_process(xyz_write()))
        data_stream_plot(plot_coup_gap())
     	data_stream(filter(sink()))
    	print "finish"	
    # print gap,coup
